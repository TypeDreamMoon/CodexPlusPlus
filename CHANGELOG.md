...
- 修复 helper 请求体上限无法调整的问题（上游 issue #2198）：转发 `/v1/responses` 时请求体超过 32 MiB 会被直接 413 拒绝，且没有任何配置项可以调大，长会话叠加多张图片时容易触发，下游网关调大也无效。现在解压后与压缩前两个上限都可配置，默认值保持 32 MiB / 64 MiB 不变。
- 设置页新增「请求体大小上限」面板（`codexPlusMaxHttpBodyMb` / `codexPlusMaxHttpEncodedBodyMb`，单位 MiB，填 0 沿用默认）；也可用环境变量 `CODEX_PLUS_MAX_HTTP_BODY_BYTES` / `CODEX_PLUS_MAX_HTTP_ENCODED_BODY_BYTES` 覆盖，优先级更高并有提示。
- 压缩前上限留空时自动取解压后上限的 2 倍，避免出现「解压后调到 128 MiB、压缩前仍卡在 64 MiB」的不一致组合。
- 413 与解压超限的报错现在会带上当前上限值和可调项名称，便于自行排查。

## 1.3.0 - 2026-09-18

- 移除全部广告与赞助推广：管理工具概览页置顶推荐位、「推荐内容」整页与侧栏入口，Codex 注入面板的「推荐内容」标签页，dream-skin 内置主题里的赞助推广数据，后端 `ads` 拉取链路（`/ads` 路由、`load_ads` 命令、直连 Ad-List 的兜底抓取），以及 README 的赞助商版块。
- 新增「更新源」设置：可在上游、本 fork 与自定义仓库之间切换，更新检查按所选仓库读取 `latest.json`；选中 fork 或自定义仓库且其尚无 Release 时自动回退到上游。
- 新增 tag 自动发版：推送 `v*` 版本 tag 即自动创建 GitHub Release，并由 CI 构建 Windows 安装包与两个 macOS DMG 作为附件。
- 版本号跟随 tag：构建前由 `tools/version-stamp.mjs` 把 tag 写入 `Cargo.toml`、`package.json` 与 `tauri.conf.json`，保证安装包名、二进制内嵌版本与 Release 标题一致。
- 排版修正：面板头部与第一块内容之间补足间距，首卡不再紧贴标题下的收口细线；配置项标签列改为按内容取宽，不同长度的标签值左边缘对齐。
